...
public class TileMapTest extends AbstractTest
{
	...
	private short[] rotations = {0, 90, 0, 0};
			...
			handlers.MouseListenerHandler mouselistenerhandler, PApplet applet)
	{
		...
		this.banks.add(new TestSpriteBank(applet));
		this.spritenames = new ArrayList<String>();
		this.spritenames.add("background");
		this.spritenames.add("background2");
		
		this.map = new TileMap(500, 300, drawer, actorhandler, null, 2, 2, 96, 
				96, this.bankindexes, this.rotations, this.xscales, this.yscales, 
				this.nameindexes);
	}
